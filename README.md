...
<p>
  ...
  <a href="https://github.com/PierreAdel/repo-radar-app/actions/workflows/ci.yml"><img alt="Lighthouse: passing" src="https://img.shields.io/badge/Lighthouse-passing-brightgreen?logo=lighthouse" /></a>
...
- **Lighthouse CI budgets** - LCP ≤ 2500ms, TBT ≤ 200ms, CLS ≤ 0.1 (blocking gates);
  ...
  preview deploy. The budget was failing earlier on (median LCP over the 2500ms cap);
  deferring Sentry init off the critical path and dropping an unnecessary chart-chunk
  preload brought it back under budget, and the Lighthouse job has been passing since.
  See the Lighthouse badge above, or [`CONSTRAINTS.md`](./CONSTRAINTS.md) for the current
  gate definitions.
...
**Codebase knowledge graph:** [`graphify-out/`](./graphify-out) holds a knowledge graph
of this repo generated by [`graphify`](https://pypi.org/project/graphifyy/) - god nodes,
community structure, and cross-file relationships - used for AI-assisted navigation and
architecture review. Regenerate it after structural changes with `graphify update .`.
...
**With more time**, the next things worth doing: add 429/`Retry-After`-aware backoff
and wire up `pnpm audit`/Dependabot.
